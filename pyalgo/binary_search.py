# 이분 탐색
"""
자료 크기 순서대로 정렬된 리스트에ㅓㅅ 특정한 값이 있는지
찾아 그 위치를 돌려주는 알고리즘

☆ 탐색 과정
1. 중간 위치를 찾는다.
2. 찾는 값과 중간 위치값을 비교한다.
3. 찾는 값이 중간 위치 값보다 크면 중간 위치의 오른쪽으로 대상으로
   탐색하고, 작으면 왼족을 대상으로 탐색한다.
"""
...
def bin_search(a, x):  # 리스트 a와 x(찾는값)
    ...
    while start <= end:  # 탐색할 범위가 있는 동안 반복
        ...
        if x == a[mid]:
            return mid  # 위치값 찾음
        elif x > a[mid]:  # 찾는 값이 중간값 보다 크면
            start = mid + 1  # 오른쪽 범위를 좁혀 계속 탐색
        else:              # 찾는 값이 중간값 보다 작으면
...
2. 49>25 25의 오른쪽 범위 탐색
3. mid=(5+8)//2=6 a[6]=49 
